...
public class FinishedMilestonesFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rvHistoryList.setAdapter(new EarnedPointHistoryAdapter(listPointHistory, getActivity(), Constants.HistoryType.MILESTONES));
        ...
        nestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
            ...
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                if (scrollY == v.getChildAt(0).getMeasuredHeight() - v.getMeasuredHeight()) {
                    if (pageNo < numOfPage) {
                        new GetEarnedPointHistoryAsync(getActivity(), Constants.HistoryType.MILESTONES, String.valueOf(pageNo + 1));
                    }
                }
            }
        });
        if (SharePreference.getInstance().getBoolean(SharePreference.IS_LOGIN)) {
            new GetEarnedPointHistoryAsync(getActivity(), Constants.HistoryType.MILESTONES, String.valueOf(pageNo));
        } else {
            hideShowList();
        }
    }
