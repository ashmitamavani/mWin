...
public class Async_Minesweeper_save {
    ...
    public Async_Minesweeper_save(final Activity activity, String points) {
        ...
        try {
            ...
            jObject.put("HJKHJKH", points);
            jObject.put("DFFFGD", SharePreference.getInstance().getString(SharePreference.userId));
            jObject.put("HJKHJH", SharePreference.getInstance().getString(SharePreference.userToken));
            jObject.put("QWSERS", SharePreference.getInstance().getString(SharePreference.AdID));
            jObject.put("JKHKHFGH", Build.MODEL);
            ...
            jObject.put("VBNVBN", SharePreference.getInstance().getString(SharePreference.AppVersion));
            jObject.put("FDGDGD", SharePreference.getInstance().getInt(SharePreference.totalOpen));
            jObject.put("DFGDFG", SharePreference.getInstance().getInt(SharePreference.todayOpen));
...
//            Logger_App.getInstance().e("Save mine ENCRYPTED ==>", AESCipher.encrypt(BuildConfig.MKEY, BuildConfig.MIV, jObject.toString()));
            ...
            Call<ApisResponse> call = apiService.saveMinesweeper(SharePreference.getInstance().getString(SharePreference.userToken), String.valueOf(n), cipher.bytesToHex(cipher.encrypt(jObject.toString())));
...
//            Logger_App.getInstance().e("RESPONSE", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(Constants.STATUS_SUCCESS)) {
                    ((MinesweeperActivity) activity).updateDataChanges(responseModel);
