...
public class GetGiveawayGiftListAsync {
    ...
    public GetGiveawayGiftListAsync(final Activity activity) {
        ...
        try {
            ...
            jObject.put("DFGDDFG", SharePreference.getInstance().getString(SharePreference.userId));
            jObject.put("VCBDFGD", SharePreference.getInstance().getString(SharePreference.userToken));

            jObject.put("SDSFDSDF", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("GHJGHJ", SharePreference.getInstance().getString(SharePreference.AdID));
            jObject.put("WERWSDFS", Build.MODEL);
            jObject.put("SDFSDFSF", Build.VERSION.RELEASE);
            jObject.put("JKLJKL", SharePreference.getInstance().getString(SharePreference.AppVersion));
            jObject.put("8797KJHJK", SharePreference.getInstance().getInt(SharePreference.totalOpen));
            jObject.put("NMHUYI", SharePreference.getInstance().getInt(SharePreference.todayOpen));
            jObject.put("SDFWERW", CommonMethodsUtils.verifyInstallerId(activity));
            jObject.put("SDFSDFE", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<ApisResponse> call = apiService.getGiveAwayList(SharePreference.getInstance().getString(SharePreference.userToken), String.valueOf(n), jObject.toString());
            ...
            if (responseModel.getStatus().equals(Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(Constants.STATUS_SUCCESS)) {
                    if (activity instanceof GiveawayGiftActivity) {
                        ((GiveawayGiftActivity) activity).setData(responseModel);
                    }
