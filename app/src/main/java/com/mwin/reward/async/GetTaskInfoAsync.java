...
public class GetTaskInfoAsync {
    ...
    public GetTaskInfoAsync(final Activity activity, String taskId) {
        this.activity = activity;
        cipher = new AESCipher();
        try {
            CommonMethodsUtils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("T7AW23", SharePreference.getInstance().getString(SharePreference.userId));
            jObject.put("QWERFR", SharePreference.getInstance().getString(SharePreference.userToken));
            jObject.put("STZLDQ", taskId);

            jObject.put("45XWZT", SharePreference.getInstance().getString(SharePreference.AdID));
            jObject.put("CIWCM2", Build.MODEL);
            jObject.put("SDAGTY", Build.VERSION.RELEASE);
            jObject.put("QW7U9K", SharePreference.getInstance().getString(SharePreference.AppVersion));
            jObject.put("WD1ANH", SharePreference.getInstance().getInt(SharePreference.totalOpen));
            jObject.put("WHD7OR", SharePreference.getInstance().getInt(SharePreference.todayOpen));
            jObject.put("T1N5CL", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            AppLogger.getInstance().e("Get TASK OFFER DETAILS ORIGINAL ==>", jObject.toString());
            Call<ApisResponse> call = apiService.getTaskDetails(SharePreference.getInstance().getString(SharePreference.userToken), String.valueOf(n), jObject.toString());
    ...
    private void onPostExecute(ApisResponse response) {
        try {
            CommonMethodsUtils.dismissProgressLoader();
            TaskInfoResponseModel responseModel = new Gson().fromJson(new String(cipher.decrypt(response.getEncrypt())), TaskInfoResponseModel.class);
            ...
            if (responseModel.getStatus().equals(Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(Constants.STATUS_SUCCESS)) {
                    if (activity instanceof TaskInfoActivity) {
                        ((TaskInfoActivity) activity).setData(responseModel);
                    }
