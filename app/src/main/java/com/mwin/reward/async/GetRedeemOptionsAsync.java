...
public class GetRedeemOptionsAsync {
    private Activity activity;
    private JSONObject jObject;
    private AESCipher cipher;

    public GetRedeemOptionsAsync(final Activity activity) {
        ...
        try {
            ...
            jObject = new JSONObject();
            jObject.put("NKJDFTJ", SharePreference.getInstance().getString(SharePreference.userId));
            jObject.put("FGHGHFG", SharePreference.getInstance().getString(SharePreference.userToken));

            jObject.put("FGYHGJN", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("MUJYLRZS", SharePreference.getInstance().getString(SharePreference.AdID));
            jObject.put("MNTYK", Build.MODEL);
            jObject.put("DGYYN", Build.VERSION.RELEASE);
            jObject.put("ARTJHN", SharePreference.getInstance().getString(SharePreference.AppVersion));
            jObject.put("IMYTEB", SharePreference.getInstance().getInt(SharePreference.totalOpen));
            jObject.put("VTHMNYU", SharePreference.getInstance().getInt(SharePreference.todayOpen));
            jObject.put("VRTYM", CommonMethodsUtils.verifyInstallerId(activity));
            jObject.put("MOPUY", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));

            Call<ApisResponse> call = apiService.getWithdrawalType(SharePreference.getInstance().getString(SharePreference.userToken), String.valueOf(n), jObject.toString());
    ...
    private void onPostExecute(ApisResponse response) {
        try {
            CommonMethodsUtils.dismissProgressLoader();
            RedeemOptionsResponseModel responseModel = new Gson().fromJson(new String(cipher.decrypt(response.getEncrypt())), RedeemOptionsResponseModel.class);
            if (responseModel.getStatus().equals(Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(Constants.STATUS_SUCCESS)) {
                    if (activity instanceof RedeemOptionsListActivity) {
                        ((RedeemOptionsListActivity) activity).setData(responseModel);
                    }
